...
class FourDirSensor():
    def __init__(self, verbose: bool = False,
        ...
        self.center = np.array([[[self.image_size[0] / 2, self.image_size[1] / 2]]], dtype=np.float32)
        ...
        self.buffer = np.zeros((self.num_dir, 2))
            ...
            for cube in self.cubes:
                if ids in cube.ids:
                    ...
                    if np.abs(rel_phi-0.5*np.pi) > 0.25*np.pi:
                        ...
                        tangent_px_origin = np.mean(cornerss[ids_cubes_i], axis=1, dtype=np.float32, keepdims=True)
                        corners_pose = tuple(coord[ii, :, :] + self.center for ii in range(len(ids_cubes_i)))
                        corners_dist = []
                        for i in range(len(ids_cubes_i)):
                            coord_t = self.fisheye_transform.spherical2tangent(corners[ids_cubes_i[i]])  + self.center
                            corners_dist.append(coord_t + (tangent_px_origin[i] - np.mean(tangent_px_origin, axis=0, keepdims=True)))

                        cube_corners.append(corners_dist)
                        ids_undist.append(ids[ids_cubes_i])
                        cubes.append(cube)

                        _, rvec_undist, _ = aruco.estimatePoseBoard(corners_pose, ids[ids_cubes_i], cube,
                                                                               self.camera_matrix,
                                                                               self.dist_coeffs,
                                                                               None,
                                                                               None)

                        _, _, t_dist = aruco.estimatePoseBoard(corners_dist, ids[ids_cubes_i], cube,
                                                                               self.camera_matrix,
                                                                               self.dist_coeffs,
                                                                               None,
                                                                               None)
                                                                               
                        rel_cube = np.linalg.norm(np.mean(t_dist[2])/len(ids_cubes_i))
                        if rel_cube < rel_r and rel_cube > 0.05:
                            rel_r = rel_cube
                            euler_ang = euler_decom(rvec_undist)
                            heading = (np.mod(bearing + euler_ang[0], 2 * np.pi) / np.pi * 180).round()[0]
                            cv2.drawFrameAxes(gray_img, self.camera_matrix, self.dist_coeff, rvec_undist, t_dist, self.aruco_marker_length)
                            aruco.drawDetectedMarkers(gray_img, corners_dist, ids[ids_cubes_i])
            print("##############")
            print(rel_r, heading)
